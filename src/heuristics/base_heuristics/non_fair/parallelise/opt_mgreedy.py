...
class OptimisedMGreedy:
    ...
    def _get_influence(self, seed_set: frozenset[int]) -> float | Any:
        """
        Get influence with efficient caching.
        """
        ...
        result = self.cascade_model.estimate_influence(
            seeds=list(seed_set),
    ...
    def _compute_marginal_gain(self, seed_set: set[int], node: int) -> float:
        """
        ...
        return self._get_influence(new_set) - self._get_influence(current_set)
    ...
    def run(self) -> set[int]:
        ...
        for node in initial_progress:
            marginal_gain = self._compute_marginal_gain(selected, node)
        ...
        while priority_queue and budget_used < self.budget:
            ...
            if current_best.iteration_updated < iteration - 1:
                new_marginal_gain = self._compute_marginal_gain(selected, current_best.node_id)
        ...
        selected_welfare = self._get_influence(frozenset(selected)) if selected else 0.0

        return selected if selected_welfare >= best_singleton_welfare else {best_singleton}


def mgreedy_optimised(
        graph: nx.Graph | nx.DiGraph,
        costs: dict[int, float],
        budget: float,
        probability: float = 0.1,
...
if __name__ == '__main__':
    ...
    seeds = mgreedy_optimised(graph, costs, budget, probability, num_sims)
