    ...
    def run(self) -> set[int]:
            ...
            if best.iteration_updated < iteration - 1:
                ...
                heapq.heappush(
                    ...
                    CELFNode(
                        ...
                        cost=best.cost,
                        marginal_gain_per_cost=new_mgpc,
        ...
        for prefix in history:
            ...
            for n in affordable:
                if n in prefix:
                    continue
                if prefix_cost + self.costs[n] <= self.budget:
                    cand = frozenset(prefix | {n})
                    w = self._get_welfare(cand)
                    if w > best_w:
                        best_w = w
                        best_set = set(cand)
            enh_bar.update(1)
            enh_bar.set_postfix(
                {
                    "prefix": len(prefix),
                    "best_seeds": len(best_set),
                    "best_welfare": f"{best_w:.4f}",
                    "inf_cache_hit": f'{self.cache_stats["influence"].hit_rate:.1%}',
                    "wel_cache_hit": f'{self.cache_stats["welfare"].hit_rate:.1%}',
                }
            )
